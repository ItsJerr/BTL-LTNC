...
class imageButton: public EventReceiver {
    string imagePath, choosenImage;
public:
    imageButton(const string& title,const string& imagePath, const string& choosenImage, const SDL_Rect& rect, const int& rimWidth, function<bool()> onClick, bool* scrFlashCheck = nullptr,
        bool* dependencies = nullptr, int WrapLength = 0)
        : title(title), rim(rect), rimColor({255, 255, 255, 255}), centerColor({0, 0, 0, 255}), textColor({255, 255, 255, 255}),
          flashColor({255, 255, 255, 255}), FlashEndAction(onClick), wrapLength(WrapLength), dependencies(dependencies),
          ButtonFlashing(scrFlashCheck), imagePath(imagePath), choosenImage(choosenImage) {
        ...
        TTF_SizeText(gFont, title.c_str(), &centerText.w, &centerText.h);
        centerText.x = center.x + (center.w - centerText.w) / 2;
        centerText.y = center.y;
    }

    void SetText(const string& title) final {
        this -> title = title;
    }
    ...
    bool HandleEvent(const SDL_Event* event) override {
        if (event->type == SDL_MOUSEBUTTONDOWN && event->button.button == SDL_BUTTON_LEFT && isHovered) {
            if (ButtonFlashing != nullptr && !*ButtonFlashing) {
                LoadImage(choosenImage);
                inFlash = *ButtonFlashing = true;
                                return true;
            }
        }
            ...
            if (isHovered != inside(event->motion.x, event->motion.y)) {
                //LoadImage(imagePath);
                isHovered = !isHovered;
                if (inFlash && dependencies) *dependencies = true;
                else if (dependencies) {
                    if (ButtonFlashing && *ButtonFlashing && !inFlash) *dependencies = false;
    ...
    void DisplayAsset() override {
        ...
        if (inFlash) {
            if ((flashFrameCounter / 10) % 2 == 0)
                {
                    SDL_SetTextureColorMod(texture, flashColor.r, flashColor.g, flashColor.b);
                    textSurface = TTF_RenderText_Solid_Wrapped(gFont, title.c_str(), flashColor, wrapLength);
                }
            else{
                SDL_SetTextureColorMod(texture, textColor.r, textColor.g, textColor.b);
                textSurface = TTF_RenderText_Solid_Wrapped(gFont, title.c_str(), textColor, wrapLength);
            }
                textTexture = SDL_CreateTextureFromSurface(gRenderer, textSurface);
            if (++flashFrameCounter == 60) {
                inFlash = false;
                if (ButtonFlashing != nullptr) *ButtonFlashing = false;
...
private:
    bool inside(int x, int y) {
        return x >= rim.x && x <= rim.x + rim.w && y >= rim.y && y <= rim.y + rim.h;
    }
    ...
    bool inFlash = false, isHovered = false;
